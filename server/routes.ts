...
const roomTokenAccess = new Map<string, Map<string, { userId: string | null; passwordVerified: boolean }>>();
...
function issueRoomToken(roomCode: string): string {
  ...
  if (!roomTokenAccess.has(roomCode)) roomTokenAccess.set(roomCode, new Map());
  roomTokenAccess.get(roomCode)!.set(token, { userId: null, passwordVerified: false });
  return token;
}
...
function setTokenAccess(roomCode: string, token: string, userId: string | null, passwordVerified: boolean) {
  if (!roomTokenAccess.has(roomCode)) roomTokenAccess.set(roomCode, new Map());
  roomTokenAccess.get(roomCode)!.set(token, { userId, passwordVerified });
}

function getTokenAccess(roomCode: string, token: string) {
  return roomTokenAccess.get(roomCode)?.get(token);
}
...
function getRequestAuth(req: Request, roomCode: string) {
  ...
  const tokenAccess = token ? getTokenAccess(roomCode, token) : undefined;
  const hasSessionIdentity = (!!session && session.roomCode === roomCode) || !!tokenAccess;
  const validToken = !!token && validateRoomToken(roomCode, token);
  const memberByUserId = isRoomMember(roomCode, userId) || isRoomMember(roomCode, tokenAccess?.userId || null);
  const memberBySession = !!session?.userId && isRoomMember(roomCode, session.userId);
  const room = storage.getRoom(roomCode);
  const passwordOk = !room?.hasPassword || !!session?.passwordVerified || !!tokenAccess?.passwordVerified;
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post("/api/rooms/:roomCode/join", (req, res) => {
    ...
    if (!room) {
      ...
      setTokenAccess(roomCode, token, verifiedUserId, true);
    ...
    const verifiedUserId = userId ? (storage.getUserById(userId) ? userId : null) : null;
    addRoomMember(roomCode, verifiedUserId);
    const passwordVerified = room.hasPassword ? !!password : true;
    setTokenAccess(roomCode, token, verifiedUserId, passwordVerified);
    const sessionId = createRoomSession(roomCode, token, verifiedUserId, passwordVerified);
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", (data: { roomCode: string; token: string; sessionId?: string } | string) => {
      ...
      const sessionFromId = sessionId ? roomSessions.get(sessionId) : undefined;
      const tokenAccess = getTokenAccess(roomCode, token);
      const session = sessionFromId && sessionFromId.roomCode === roomCode
        ? sessionFromId
        : (tokenAccess ? {
          sessionId: socket.id,
          roomCode,
          ...
          token,
          passwordVerified: tokenAccess.passwordVerified,
        } : undefined);

      if (!validateRoomToken(roomCode, token) || !session || (sessionFromId && session.token !== token)) {
